...
const About = () => {
  ...
  useEffect(() => {
    ...
    Promise.allSettled([client.fetch(aboutsQuery), client.fetch(aboutMeQuery)])
      .then(([aboutsResult, aboutMeResult]) => {
        if (aboutsResult.status === "fulfilled") {
          setAbouts(aboutsResult.value);
        }
        if (aboutMeResult.status === "fulfilled" && aboutMeResult.value) {
          setAboutMe(aboutMeResult.value);
        }
      })
      .finally(() => {
        setIsLoading(false);
  ...
  return (
    <>
      <h2 className="head-text">
        I know that <span> Building Scalable Digital Solutions</span>
        <br />
        That Drive <span> Business Success.</span>
      </h2>
